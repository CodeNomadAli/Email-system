              ...
              <div className='flex gap-2 items-center overflow-hidden'>
                ...
                <div className='flex items-center flex-wrap gap-2 overflow-hidden'>
                  <Typography color='text.primary' noWrap>
                    {currentEmail.subject ? currentEmail.subject : "this is the Empty emails"}
                  </Typography>
                  <div className="flex items-center flex-wrap gap-2">
                    {currentEmail.labels?.slice(0, 3).map((label) => (
                      <CustomChip
                        key={label}
                        variant="tonal"
                        round="true"
                        size="small"
                        label={label}
                        color={labelColors[label.toLowerCase()]?.color ?? 'default'}
                        className="capitalize"
                      />
                    ))}

                    {currentEmail.labels && currentEmail.labels.length > 3 && (
                      <CustomChip
                        variant="tonal"
                        round="true"
                        size="small"
                        label={`+${currentEmail.labels.length - 3}`}
                        color="default"
                      />
                    )}
                  </div>
